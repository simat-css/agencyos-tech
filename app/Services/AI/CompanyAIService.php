    ...
    public function process(string $command): array
    {
        ...
        */
        if (str_contains(strtolower($command), 'create company')) {
         ...
         $existingCompany = Company::withTrashed()
    ->where(
        'email',
        $data['email']
    )
...
if (str_starts_with(strtolower($command), 'activate company')) {
    ...
    return $this->requestCompanyActivateConfirmation(
        $command
    );
}
...
if (isset($updateData['email']) && $this->companyService
    ->emailExistsExcept(
        $updateData['email'],
        $company->id
    )) {
...
private function requestCompanyActivateConfirmation(
    string $command
): array {
...
private function confirmCompanyActivate(
    string $command
): array {

    $pending =
        session('pending_company_activate');
    ...
    if (!$pending) {
        ...
        return [
            'success' => false,
            'message' =>
                'No pending activation request found.'
        ];
...
if ($userCount > 0) {
    ...
    return [
        'success' => false,
        'message' =>
            "Company {$company->name} contains {$userCount} users. Remove or reassign users before deleting the company."
    ];
}
...
private function requestCompanyRestoreConfirmation(
    string $command
): array {
    ...
    if (
        ...
        return [
            'success' => false,
            'message' =>
            'You do not have permission to restore companies.'
        ];
    }
    ...
    if(!$company) {
        ...
        return [
            'success'=>false,
            'message'=>
            "Deleted company {$companyName} not found."
        ];
...
private function confirmCompanyRestore(
    string $command
): array {
